...
<div class="flex items-center gap-4 mb-10">
    <a href="{{ route('admin.comensales.index') }}" class="w-10 h-10 rounded-full bg-surface-container-high flex items-center justify-center hover:bg-primary-container hover:text-white transition-all">
        <span class="material-symbols-outlined">arrow_back</span>
    </a>
    <div>
        <h2 class="text-4xl font-extrabold text-on-surface tracking-tight mb-2">Editar Comensal</h2>
        <p class="text-on-surface-variant font-medium">Modifica los detalles del usuario: <span class="font-bold border-b border-primary">{{ $comensal->nombre }}</span></p>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <select name="estado" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" required>
                    <option value="activo" {{ $comensal->estado === 'activo' ? 'selected' : '' }}>Activo</option>
                    <option value="inactivo" {{ $comensal->estado === 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                    <option value="baneado" {{ $comensal->estado === 'baneado' ? 'selected' : '' }}>Baneado</option>
                </select>
            </div>

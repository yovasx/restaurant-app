<form action="{{ route('admin.restaurantes.update', $usuario->id) }}" method="POST" class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div class="space-y-2">
            <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Nombre o Razón Social</label>
            <input name="nombre" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" type="text" required data-modal-initial-focus value="{{ old('nombre', $usuario->nombre) }}" />
        </div>

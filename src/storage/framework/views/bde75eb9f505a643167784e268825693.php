...
<form action="<?php echo e(route('admin.usuarios.store')); ?>" method="POST" class="space-y-6">
    ...
    <?php if($fixedRole): ?>
        <input type="hidden" name="user_type" value="<?php echo e($fixedRole); ?>">
        <div class="rounded-3xl bg-primary-fixed px-5 py-4 text-primary">
            ...
            <p class="mt-1 font-headline text-xl font-extrabold"><?php echo e($roleLabels[$fixedRole] ?? 'Usuario'); ?></p>
        </div>
    <?php endif; ?>
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div class="space-y-2">
            ...
            <input name="nombre" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" type="text" required data-modal-initial-focus value="<?php echo e(old('nombre')); ?>" placeholder="Nombre o Razón Social" />
        </div>
        ...
        <div class="space-y-2 apellido-field">
            ...
            <input name="apellido_paterno" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" type="text" value="<?php echo e(old('apellido_paterno')); ?>" placeholder="Solo para comensal"/>
        </div>

        <div class="space-y-2 apellido-field">
            <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Apellido Materno</label>
            <input name="apellido_materno" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" type="text" value="<?php echo e(old('apellido_materno')); ?>" placeholder="Solo para comensal"/>
        </div>

        <div class="space-y-2">
            <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Correo Electrónico</label>
            <input name="email" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" type="email" required value="<?php echo e(old('email')); ?>" placeholder="usuario@correo.com"/>
        </div>

        <div class="space-y-2">
            <label class="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Teléfono</label>
            <input name="telefono" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" type="text" required value="<?php echo e(old('telefono')); ?>" placeholder="70000000"/>
        </div>
        ...
        <div class="space-y-2 nit-restaurante-field">
            ...
            <input name="nit" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" type="text" value="<?php echo e(old('nit')); ?>" placeholder="Solo para restaurante"/>
        </div>
        ...
        <?php if(!$fixedRole): ?>
            <div class="space-y-2">
                ...
                <select name="user_type" class="w-full bg-surface-container-low border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-primary font-medium text-on-surface" required>
                    <option value="" disabled <?php echo e(!$selectedRole ? 'selected' : ''); ?>>Selecciona un rol</option>
                    <option value="1" <?php echo e($selectedRole == '1' ? 'selected' : ''); ?>>Administrador (staff)</option>
                    <option value="2" <?php echo e($selectedRole == '2' ? 'selected' : ''); ?>>Restaurante (negocio)</option>
                    <option value="comensal" <?php echo e($selectedRole == 'comensal' ? 'selected' : ''); ?>>Comensal (cliente)</option>
                </select>
            </div>
        <?php endif; ?>

        <div class="space-y-2 <?php echo e($fixedRole ? 'md:col-span-1' : 'md:col-span-2'); ?>">
...
<?php /**PATH C:\Users\yovan\Proyecto-restaurant\src\resources\views/admin/usuarios/_modal-form.blade.php ENDPATH**/ ?>